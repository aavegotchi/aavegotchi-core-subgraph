...
interface SetsReturn {
    setFound: FoundSet;
    traits: number[];
    rarityScore: number;
}

export function modifyWithAavegotchiSets(
    equipped: number[],
    traits: number[],
    rarityScore: number
): SetsReturn {
    ...
    let setFound: FoundSet = null;
    for (const wearableSet of setData) {
        const setWearableIds = wearableSet.wearableIds.reduce(
            getEquipmentIds,
            []
        );
        if (
            setWearableIds.every((wearableId) =>
                equipped.includes(wearableId)
            ) &&
    ...
    if (setFound != null) {
        rarityScore = rarityScore + setFound.traitsBonuses[0];
        for (let i = 1; i < 5; i++) {
            ...
            const rarityScoreChange =
                Math.abs(finalTraitValue - 50) - Math.abs(oldTraitValue - 50);
            rarityScore = rarityScore + rarityScoreChange;
        }
    }
