...
import { handleMintPortals } from "../src/mappings/aavegotchi";
...
test("handleMintPortals - should update portal status and stats entity", () => {
    ...
    let _to = new ethereum.EventParam("_to",  ethereum.Value.fromAddress(newMockevent.address));
    ...
    let _numAavegotchisToPurchase = new ethereum.EventParam("_numAavegotchisToPurchase", ethereum.Value.fromUnsignedBigInt(BIGINT_ONE));
    event.parameters.push(_numAavegotchisToPurchase);

    let _hauntId = new ethereum.EventParam("_hauntId", ethereum.Value.fromUnsignedBigInt(BIGINT_ONE));
